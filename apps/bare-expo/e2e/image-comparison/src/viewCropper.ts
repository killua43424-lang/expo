...
import { ScreenInspectorIOS } from '../viewshot-dylib/ScreenInspectorIOS';
...
async function getCoordinatesViaDylib(
  ...
  const requestPipePath = '/tmp/ios_screenshot_request';
  const responsePipePath = '/tmp/ios_screenshot_response';

  // Check if pipes exist (dylib is running)
  if (!fs.existsSync(requestPipePath) || !fs.existsSync(responsePipePath)) {
    console.log('⚠️  Dylib pipes not found, skipping dylib method');
    return null;
  }
  ...
  try {
    ...
    console.log(`✅ Got coordinates via dylib: ${JSON.stringify(bounds)}`);
    return {
      x: bounds.x * 3,
      y: bounds.y * 3,
